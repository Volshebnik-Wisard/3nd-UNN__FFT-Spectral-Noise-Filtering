...
public:
	...
	void DrawGrid(vector<double> y_first_graph, int typefunc, vector<double> x_first_graph, int N, double A, double x_max2)
	{
		if (!init) return;

		CPen subgrid_pen(PS_DOT, 1, RGB(170, 170, 170));
	...
	void DrawAnimation(vector<double>& y_first_graph, int typefunc, vector<double>& x_first_graph, int N, double A, double x_max2, int pen_number)
	{
		if (!init) return;
		...
		CPen data_pen1(PS_SOLID, 2, RGB(222, 12, 98));
		CPen data_pen2(PS_SOLID, 2, RGB(90, 170, 160));
		CPen data_pen3(PS_SOLID, 2, RGB(84, 0, 153));
